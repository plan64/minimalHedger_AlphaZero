...
from hedger_TV.hedgerGame_TV import HedgerPlan_TV
from MCTS import MCTS


n_testPaths = 20
#Set below the path of the config file to be used for testing
config_file_path = Path(os.getcwd()) / r'ZZZ_Config/config_TV.yaml'
...
scenario = HedgerPlan_TV(CONFIG['episodes'])
...
if(False): #mcts versus bsm deltas
    mctsHedges = np.cumsum(mcts_actions_h,axis=1)
    plt.scatter(100*stockPrices_h[:,30], mctsHedges[:,30], s=3, c = 'b', label = 'MCTS-Hedger')
    plt.xlim(60,150)
    plt.ylim(-0.51,1.5)
    plt.legend(loc='upper left', prop ={'size':10})
    figure = plt.gcf()
